...
export class UserConversationService {
  ...
  async updateLastMessageId(userConversationId: string, lastMessageId: string) {
    ...
    if (!updateMessageId) {
      console.log(`Can't update last message id for user conversation ${userConversationId}`);
      return null;
    }
  ...
  async getMessage(userConversationId: string) {
    const userConversation = await this.userConversationModel.findById(userConversationId);
